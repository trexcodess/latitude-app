...
import { BattleSession, UserProfile, UserTier, BattlePhase, ChatMessage, ViewState } from '../types';
...
const Backstage: React.FC<{ user: UserProfile | null, setView: (view: ViewState) => void; }> = ({ user, setView }) => {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-12 animate-fade-in relative pb-32 bg-white dark:bg-black transition-colors">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center mb-16 relative z-10 gap-8">
        <div>
          <h2 className="text-5xl sm:text-6xl md:text-8xl font-black uppercase tracking-tighter italic text-black dark:text-white">The <span className="text-latitude-red">Ring</span></h2>
      ...
      <div className="grid lg:grid-cols-4 gap-8 relative z-10">
        <div className="lg:col-span-3 space-y-8">
           ...
           <div className="flex flex-wrap gap-4 justify-center">
              <button onClick={() => handleTip(0.1)} className="px-6 py-3 sm:px-8 sm:py-4 bg-latitude-solana text-black font-black uppercase rounded-2xl text-[10px] hover:scale-105 transition-all shadow-lg border-b-4 border-green-900">Tip 0.1 SOL</button>
              <button onClick={() => handleTip(0.5)} className="px-6 py-3 sm:px-8 sm:py-4 bg-latitude-solana text-black font-black uppercase rounded-2xl text-[10px] hover:scale-105 transition-all shadow-lg border-b-4 border-green-900">Tip 0.5 SOL</button>
              <button onClick={() => handleTip(1.0)} className="px-6 py-3 sm:px-8 sm:py-4 bg-latitude-red text-white font-black uppercase rounded-2xl text-[10px] hover:scale-105 transition-all shadow-lg border-b-4 border-red-950">Tip 1.0 SOL</button>
           </div>
        </div>
      ...
      {isDjMode && (
        <div className="fixed bottom-0 left-0 w-full z-50 bg-white dark:bg-[#080808] border-t-4 border-latitude-teal p-6 sm:p-10 shadow-2xl animate-slide-up">
            <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-8 sm:gap-12 items-center">
                <div className="flex-1 text-center md:text-left">
                   <h4 className="text-xl sm:text-2xl font-black uppercase italic text-black dark:text-white mb-2">DJ Broadcast Hub</h4>
                   <p className="text-[9px] text-gray-500 uppercase font-mono tracking-widest">PRO_LINK v4.0 Active</p>
                </div>
                <div className="flex gap-4">
                    {!isBroadcasting ? (
                      <button onClick={startBroadcast} className="px-8 sm:px-12 py-4 sm:py-5 bg-latitude-solana text-black font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl border-b-4 border-green-900">Start Broadcast</button>
                    ) : (
                      <button onClick={stopBroadcast} className="px-8 sm:px-12 py-4 sm:py-5 bg-latitude-red text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl border-b-4 border-red-950">Kill Signal</button>
                    )}
                </div>
            </div>
        </div>
